...
class QualityControlAgent(BaseAgent):
    ...
    def quality_control_node(self, state: AgentState):
        print("--- 节点: 质检总结员 ---")
        ...
        try:
            ...
            final_summary_response = self.llm.with_structured_output(ReplyResult).invoke(
                formatted_prompt,
            ...
            print(f"!!! AI生成总结失败: {e}")
            final_summary_response = ReplyResult(reply_context="感谢您的咨询，本次服务已结束。如果还有其他问题，欢迎随时联系我们！", summarize_context="", can_reply_to_user=True)
        ...
        # 将 AIMessage 添加到消息列表
        new_messages = list(messages) + [AIMessage(content=reply_text)]
